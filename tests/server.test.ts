...
describe("mcp-browser-buddy server", () => {
  it("responds to initialize and tools/list over stdio", async () => {
    const result = await new Promise<{ stdoutLines: string[] }>((resolve, reject) => {
    ...
    const pkgVersion = (await import("../package.json", { with: { type: "json" } })).default
      .version;
